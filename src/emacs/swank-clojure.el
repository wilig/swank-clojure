...
;; Package-Requires: ((slime-repl "20091016") (clojure-mode "1.5"))
;;
...
(defvar swank-clojure-version "1.0-SNAPSHOT")

;; TODO: should this use 1.0.0 by default?
(defcustom swank-clojure-clojure-version "1.1.0-alpha-SNAPSHOT"
  "The version of Clojure to use."
  :type 'string
  :group 'swank-clojure
  :options '("1.0.0" "1.1.0-alpha-SNAPSHOT"))

(defcustom swank-clojure-contrib-version "1.0-SNAPSHOT"
  "The version of Clojure Contrib to use."
  :type 'string
  :group 'swank-clojure
  :options '("1.0-compat-SNAPSHOT" "1.0-SNAPSHOT"))
...
(defvar swank-clojure-mvn-path-template
  "~/.m2/repository/org/clojure/%s/%s/%s-%s.jar"
  "Template for path to Clojure jar files inside local Maven repository.
Paths are generated by `format' with args: name version name version.")

(defun swank-clojure-mvn-path (artifact)
  (let ((name (car artifact))
        (version (cdr artifact)))
    (format swank-clojure-mvn-path-template name version name version)))

(defcustom swank-clojure-classpath 
  (append 
   (when (file-directory-p "~/.clojure")
     (directory-files "~/.clojure" t ".jar$"))
   (mapcar 'swank-clojure-mvn-path
           `(("clojure"         . ,swank-clojure-clojure-version)
             ("clojure-contrib" . ,swank-clojure-contrib-version))))
  "The classpath from which clojure will load from (passed into
java as the -cp argument). On default, it includes all jar files
within ~/.clojure/ as well as Clojure and Contrib."
  :type 'list
  :group 'swank-clojure)
...
(defun swank-clojure-init (file encoding)
  (concat
   ;; TODO: is this necessary if you're using mvn-installed swank-clojure?
       ...
       (append
        ...
        (list
         ...
         (swank-clojure-concat-paths
          (cons (swank-clojure-mvn-path (cons "swank-clojure"
